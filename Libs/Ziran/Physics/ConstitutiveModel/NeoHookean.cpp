...
template <class T, int _dim>
void NeoHookean<T, _dim>::updateScratchSVD(const TM& new_F, Scratch& s) const // private
{
    s.F = new_F;
...
template <class T, int _dim>
void NeoHookean<T, _dim>::updateScratch(const TM& new_F, Scratch& s)
{
    using std::log;
    ...
    if constexpr (dim == 2) {
        ...
        s.isotropic.psi0 = mu * (s.sigma(0) - inv0) + lambda * inv0 * s.logJ;
        s.isotropic.psi1 = mu * (s.sigma(1) - inv1) + lambda * inv1 * s.logJ;

        T inv2_0 = T(1) / s.sigma(0) / s.sigma(0);
        T inv2_1 = T(1) / s.sigma(1) / s.sigma(1);

        s.isotropic.psi00 = mu * (T(1) + inv2_0) - lambda * inv2_0 * (s.logJ - T(1));
        s.isotropic.psi11 = mu * (T(1) + inv2_1) - lambda * inv2_1 * (s.logJ - T(1));
        s.isotropic.psi01 = lambda / s.J;

        // (psi0-psi1)/(sigma0-sigma1)

        s.isotropic.m01 = mu + (mu - lambda * s.logJ) / s.J;
        ...
        s.isotropic.psi0 = mu * (s.sigma(0) - inv0) + lambda * inv0 * s.logJ;
        s.isotropic.psi1 = mu * (s.sigma(1) - inv1) + lambda * inv1 * s.logJ;
        s.isotropic.psi2 = mu * (s.sigma(2) - inv2) + lambda * inv2 * s.logJ;
        ...
        s.isotropic.psi00 = mu * (T(1) + inv2_0) - lambda * inv2_0 * (s.logJ - T(1));
        s.isotropic.psi11 = mu * (T(1) + inv2_1) - lambda * inv2_1 * (s.logJ - T(1));
        s.isotropic.psi22 = mu * (T(1) + inv2_2) - lambda * inv2_2 * (s.logJ - T(1));
...
template <class T, int _dim>
void NeoHookean<T, _dim>::firstPiolaDerivative(const Scratch& ss, Hessian& dPdF) const
{
    if constexpr (dim == 2) {
