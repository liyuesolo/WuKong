...
void Foam3DApp::generateScenario() {
    ...
    VectorXd vertices((8 + 10) * 3);
    vertices << VectorXd::Random(10 * 3), infbox;
    VectorXd params = VectorXd::Zero(8 + 10);
...
//        bool orientation = (c1 - c0).dot(camera_pos - c0) > 0;
//
...
//            e++;
//        }
//        for (int i = 0; i < face.nodes.size() - 2; i++) {
//            if (orientation) {
//                F(f, 0) = nodeIndices.at(face.nodes[i]);
//                F(f, 1) = nodeIndices.at(face.nodes[i + 1]);
//                F(f, 2) = nodeIndices.at(face.nodes[face.nodes.size() - 1]);
//                Fc.row(f) = colors.row(cell);
//            } else {
//                F(f, 0) = nodeIndices.at(face.nodes[i]);
//                F(f, 2) = nodeIndices.at(face.nodes[i + 1]);
//                F(f, 1) = nodeIndices.at(face.nodes[face.nodes.size() - 1]);
//                Fc.row(f) = colors.row(cell);
//            }
//            f++;
//        }
