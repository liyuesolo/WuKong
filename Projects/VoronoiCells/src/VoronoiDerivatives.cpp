    ...
    const std::vector<int>& site_indices, MatrixXT& dx_ds, bool reduced)
{
    ...
    if (site_indices.size() == 2)
    {
        ...
        if (zero_idx == -1)
        {
            ...
            // Vector<T, 4> dDdwxsi, dDdwxsj;
            // computeGeodesicLengthGradient(x, si, dDdwxsi);
            // computeGeodesicLengthGradient(x, sj, dDdwxsj);

            
            // TV2 dgdx_tilde; dgdx_tilde.setZero();

            // dgdx_tilde += dDdwxsi.segment<2>(0) - dDdwxsj.segment<2>(0);

            // Vector<T, 4> dgds;
            // dgds.segment<2>(0) = dDdwxsi.segment<2>(2);
            // dgds.segment<2>(2) = -dDdwxsj.segment<2>(2);

            // Matrix<T, 2, 4> ATb = dgdx_tilde * dgds.transpose();
            // Matrix<T, 2, 2> ATA = dgdx_tilde * dgdx_tilde.transpose();

            // Matrix<T, 2, 4> dx_tilde_ds = ATA.colPivHouseholderQr().solve(-ATb);
