...
void Tiling3D::fetchUnitCellFromOneFamily(int IH, std::vector<std::vector<TV2>>& eigen_polygons,
...
void Tiling3D::generatePeriodicMesh(std::vector<std::vector<TV2>>& polygons, std::vector<TV2>& pbc_corners)
{
    ...
    // for (int i = 0; i < polygons.size(); i++)
    // {
    //     TV x0(polygons[i][0][0], polygons[i][0][1], 0);
    //     TV x1(polygons[i][1][0], polygons[i][1][1], 0);
    //     TV x2(polygons[i][2][0], polygons[i][2][1], 0);
    //     TV e0 = x0 - x1, e1 = x2 - x1;
    //     TV normal = e0.cross(e1).normalized();
    //     if (normal.dot(TV(0, 0, 1)) < 0)
    //         orientation.push_back(1);
    //     else
    //         orientation.push_back(0);
    //     std::cout << polygons[i].size() << std::endl;
    // }

    // for (int ori : orientation)
    //     std::cout << ori << " ";
    // std::cout << std::endl;
    // std::getchar();
...
void Tiling3D::getPBCUnit(VectorXT& vertices, EdgeList& edge_list)
{
