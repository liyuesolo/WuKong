...
void CellFunctionCentroidYTarget::addValue(const VectorXT &site, const VectorXT &nodes, double &value) const {
    double area = 0;
    area_function.addValue(site, nodes, area);
    double centroid = 0;
    centroid_function.addValue(site, nodes, centroid);

    value += pow(site(1) - centroid / area, 2.0);
}
...
void CellFunctionCentroidYTarget::addGradient(const VectorXT &site, const VectorXT &nodes, VectorXT &gradient_c,
    ...
    gradient_c(1) += 2 * (site(1) - centroid / area);
    gradient_x += 2 * (site(1) - centroid / area) *
...
void CellFunctionCentroidYTarget::addHessian(const VectorXT &site, const VectorXT &nodes, MatrixXT &hessian) const {
    double area = 0;
    area_function.addValue(site, nodes, area);
    double centroid = 0;
    centroid_function.addValue(site, nodes, centroid);
