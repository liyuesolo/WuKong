...
void CellFunctionAreaBarrier::addValue(const VectorXT &site, const VectorXT &nodes, double &value) const {
    double area = 0;
...
void CellFunctionAreaBarrier::addHessian(const VectorXT &site, const VectorXT &nodes, MatrixXT &hessian) const {
    double area = 0;
    area_function.addValue(site, nodes, area);
    ...
    hess += epsilon * (exponent * (exponent - 1.0) * pow(area, exponent - 2.0) * area_gradient_x * area_gradient_x.transpose() + exponent * pow(area, exponent - 1.0) * area_hess);
