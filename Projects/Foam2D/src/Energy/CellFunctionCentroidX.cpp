...
CellFunctionCentroidX::addValue(const VectorXT &site, const VectorXT &nodes, const VectorXi &next, double &value,
    ...
    weighted_mean_function.addValue(site, nodes, next, weighted_mean, cellInfo);
...
void CellFunctionCentroidX::addGradient(const VectorXT &site, const VectorXT &nodes, const VectorXi &next,
    ...
    weighted_mean_function.addGradient(site, nodes, next, temp, weighted_mean_gradient, cellInfo);

    gradient_x += weighted_mean_gradient / area - weighted_mean * area_gradient / pow(area, 2);
}

void CellFunctionCentroidX::addHessian(const VectorXT &site, const VectorXT &nodes, const VectorXi &next,
                                       MatrixXT &hessian,
                                       const CellInfo *cellInfo) const {
    double area = 0;
    area_function.addValue(site, nodes, next, area, cellInfo);
    ...
    weighted_mean_function.addHessian(site, nodes, next, weighted_mean_hessian, cellInfo);
