...
static TV3 getCircumcentre(const TV3 &v1, const TV3 &v2, const TV3 &v3) {
    ...
    double c2 = (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1 + z2 - z1) / (2 * (x2 - x1));
    double m3 = -(y3 - y1) / (x3 - x1);
    double c3 = (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1 + z3 - z1) / (2 * (x3 - x1));
...
VectorXi Power::powerDualNaive(const VectorXT &vertices) {
...
VectorXi Power::getDualGraph(const VectorXT &vertices, const VectorXT &params) {
    ...
    return powerDualNaive(vertices3d);
}
