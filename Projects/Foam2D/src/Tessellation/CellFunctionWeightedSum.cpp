...
void CellFunctionWeightedSum::addValue(const VectorXT &site, const VectorXT &nodes, double &value,
                                       const CellInfo *cellInfo) const {
    for (size_t i = 0; i < functions.size(); i++) {
        double func_value = 0;
        functions[i]->addValue(site, nodes, func_value, cellInfo);
...
void CellFunctionWeightedSum::addGradient(const VectorXT &site, const VectorXT &nodes, VectorXT &gradient_c,
    ...
    for (size_t i = 0; i < functions.size(); i++) {
        ...
        functions[i]->addGradient(site, nodes, func_gradient_c, func_gradient_x, cellInfo);
        gradient_c += func_gradient_c * weights[i];
        gradient_x += func_gradient_x * weights[i];
    }
}

void CellFunctionWeightedSum::addHessian(const VectorXT &site, const VectorXT &nodes, MatrixXT &hessian,
                                         const CellInfo *cellInfo) const {
    for (size_t i = 0; i < functions.size(); i++) {
        MatrixXT func_hessian = MatrixXT::Zero(hessian.rows(), hessian.cols());
        functions[i]->addHessian(site, nodes, func_hessian, cellInfo);
