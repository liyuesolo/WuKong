#include "../../include/Boundary/BezierCurveBoundary.h"
#include <cmath>

void BezierCurveBoundary::computeVertices() {
    int ncp = p.rows() / 2;
    v.resize(ncp * 2);
    q.resize(ncp * 2);
...
void BezierCurveBoundary::computeGradient() {
...
void BezierCurveBoundary::computeHessian() {
...
bool BezierCurveBoundary::checkValid() {
    int n_vtx = v.rows() / 2;
    for (int i = 0; i < n_vtx; i++) {
