...
template <int dim>
void SimulationApp<dim>::appendCylindersToEdges(const std::vector<std::pair<TV3, TV3>>& edge_pairs, 
...
template <int dim>
void SimulationApp<dim>::setViewer(igl::opengl::glfw::Viewer& viewer,
    igl::opengl::glfw::imgui::ImGuiMenu& menu)
{
    ...
    menu.callback_draw_viewer_menu = [&]()
    {
        ...
        if (ImGui::CollapsingHeader("Visualization", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (ImGui::Checkbox("quadratic", &solver.quadratic))
            {
                updateScreen(viewer);
            }
        }
