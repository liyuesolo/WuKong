...
public:
    ...
    /** default constructor */
    IpoptSolver(FEMSolver& _solver, const std::string& _data_folder)
        : solver(_solver), data_folder(_data_folder)
    {
        variable_num = solver.num_nodes * 2;
        ...
        Ipopt::Number& obj_value)
    {
        ...
        T E = 0.0;
        T alpha = solver.computeInversionFreeStepsize(u, solver.u);
        T ipc_step_size = solver.computeCollisionFreeStepsize(u, solver.u);
        alpha = std::min(alpha, ipc_step_size);
        E = solver.computeTotalEnergy(alpha * u);
        ...
        Ipopt::Number* grad_f)
    {
        ...
        T ipc_step_size = solver.computeCollisionFreeStepsize(u, solver.u);
        alpha = std::min(alpha, ipc_step_size);
        solver.computeResidual(alpha * u, residual);
        if (solver.use_ipc)
            solver.updateIPCVertices(alpha * u);
