...
data_folder = "/home/yueli/Documents/ETH/SandwichStructure/TrainingData/SingleStructureUniaxialStrain/"

data_all, label_all = loadDataSplitTest(data_folder + "training_data.txt")
...
def numeric_compare(x, y):
    return np.linalg.norm(test_data[x]) - np.linalg.norm(test_data[y])
...
def trainStep(opt, lambdas, sigmas, model, train_vars):
    rest_configuration = tf.convert_to_tensor([[1.0, 1.0, 0.0]])
...
def train():
    ...
    test_error, dedlambda, Psi = testStep(test_data, test_label, model)
    ...
    sigma_xx = [dedlambda[i][0] for i in range(len(test_data))]
    # plt.title("StVK test data")
    plt.plot(data_point, sigma_xx, linewidth=1.5, label = "Sigma_xx")
    plt.plot(data_point, sigma_xx_gt, linewidth=1.5, label = "GT Sigma_xx")
