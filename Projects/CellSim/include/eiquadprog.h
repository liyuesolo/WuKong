 ...
 * precomputed */
double solve_quadprog2(LLT<MatrixXd, Lower> &chol, double c1, VectorXd &g0, const MatrixXd &CE, const VectorXd &ce0, const MatrixXd &CI, const VectorXd &ci0,
                       VectorXd &x);

/* solve_quadprog is used for on-demand QP solving */
inline double solve_quadprog(MatrixXd &G, VectorXd &g0, const MatrixXd &CE, const VectorXd &ce0, const MatrixXd &CI, const VectorXd &ci0, VectorXd &x) {
 ...
 * pre-computed */
inline double solve_quadprog2(LLT<MatrixXd, Lower> &chol, double c1, VectorXd &g0, const MatrixXd &CE, const VectorXd &ce0, const MatrixXd &CI,
...
inline bool add_constraint(MatrixXd &R, MatrixXd &J, VectorXd &d, int &iq, double &R_norm) {
    ...
    for (j = n - 1; j >= iq + 1; j--) {
        /* The Givens rotation is done with the matrix (cc cs, cs -cc).
           ...
           with element (j - 1). Hence we don't have to do anything. If cc is
           zero, then we just have to switch column (j) and column (j - 1) of J.
           Since we only switch columns in J, we have to be careful how we
                         update d depending on the sign of gs.
                         Otherwise we have to apply the Givens rotation to these
