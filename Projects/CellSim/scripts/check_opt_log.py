...
def loadSQPLogCheckConvergence(base_dir):
    log_file = base_dir + "log.txt"
    iterations = []
    residual_norms = []
    energy = []
    for line in open(log_file).readlines():
        if "[SQP]" in line.strip() and "\t" not in line:
            iteration = line.split("|g|")[0].split("iter ")[-1]
    ...
    fig, axs = plt.subplots(1, 2)
    fig.suptitle('SQP log')
    axs[0].plot(iterations, residual_norms)
    axs[0].set_title('residual norm')
    axs[1].plot(iterations, energy)
    axs[1].set_title('energy')
    fig.set_size_inches(14, 10)
    plt.savefig(base_dir + "SQP_log.png", dpi=300)

if __name__ == "__main__":
    base_dir = "/home/yueli/Documents/ETH/WuKong/output/cells/79/"
