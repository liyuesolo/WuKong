#include "Repulsion.h"

void computeRepulsion2DCubicEnergy(const Eigen::Matrix<double,2,1> & xi, const Eigen::Matrix<double,2,1> & xj, double dhat, double& energy){
	double _i_var[10];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (_i_var[0])*(_i_var[0]);
	_i_var[3] = (_i_var[1])*(_i_var[1]);
	_i_var[4] = (_i_var[3])+(_i_var[2]);
	_i_var[5] = std::sqrt(_i_var[4]);
	_i_var[6] = (_i_var[5])-(dhat);
	_i_var[7] = 3;
	_i_var[8] = -(_i_var[6]);
	_i_var[9] = std::pow(_i_var[8],_i_var[7]);
	energy = _i_var[9];
}
void computeRepulsion2DCubicEnergyGradient(const Eigen::Matrix<double,2,1> & xi, const Eigen::Matrix<double,2,1> & xj, double dhat, Eigen::Matrix<double, 4, 1>& energygradient){
	double _i_var[24];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (_i_var[0])*(_i_var[0]);
	_i_var[3] = (_i_var[1])*(_i_var[1]);
	_i_var[4] = (_i_var[3])+(_i_var[2]);
	_i_var[5] = std::sqrt(_i_var[4]);
	_i_var[6] = (_i_var[5])-(dhat);
	_i_var[7] = -(_i_var[6]);
	_i_var[8] = 2;
	_i_var[9] = (_i_var[7])*(_i_var[7]);
	_i_var[10] = 3;
	_i_var[11] = (_i_var[8])*(_i_var[5]);
	_i_var[12] = 1;
	_i_var[13] = -1;
	_i_var[14] = (_i_var[10])*(_i_var[9]);
	_i_var[15] = (_i_var[12])/(_i_var[11]);
	_i_var[16] = (_i_var[14])*(_i_var[13]);
	_i_var[17] = (_i_var[16])*(_i_var[15]);
	_i_var[18] = (_i_var[17])*(_i_var[1]);
	_i_var[19] = (_i_var[17])*(_i_var[0]);
	_i_var[20] = (_i_var[8])*(_i_var[18]);
	_i_var[21] = (_i_var[8])*(_i_var[19]);
	_i_var[22] = (_i_var[20])*(_i_var[13]);
	_i_var[23] = (_i_var[21])*(_i_var[13]);
	energygradient(0,0) = _i_var[20];
	energygradient(1,0) = _i_var[21];
	energygradient(2,0) = _i_var[22];
	energygradient(3,0) = _i_var[23];
}
void computeRepulsion2DCubicEnergyHessian(const Eigen::Matrix<double,2,1> & xi, const Eigen::Matrix<double,2,1> & xj, double dhat, Eigen::Matrix<double, 4, 4>& energyhessian){
	double _i_var[44];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (_i_var[0])*(_i_var[0]);
	_i_var[3] = (_i_var[1])*(_i_var[1]);
	_i_var[4] = (_i_var[3])+(_i_var[2]);
	_i_var[5] = std::sqrt(_i_var[4]);
	_i_var[6] = 2;
	_i_var[7] = (_i_var[6])*(_i_var[5]);
	_i_var[8] = (_i_var[7])*(_i_var[7]);
	_i_var[9] = 1;
	_i_var[10] = (_i_var[5])-(dhat);
	_i_var[11] = (_i_var[9])/(_i_var[8]);
	_i_var[12] = -(_i_var[10]);
	_i_var[13] = -(_i_var[11]);
	_i_var[14] = (_i_var[12])*(_i_var[12]);
	_i_var[15] = 3;
	_i_var[16] = (_i_var[9])/(_i_var[7]);
	_i_var[17] = (_i_var[13])*(_i_var[6]);
	_i_var[18] = -1;
	_i_var[19] = (_i_var[15])*(_i_var[14]);
	_i_var[20] = (_i_var[6])*(_i_var[12]);
	_i_var[21] = (_i_var[17])*(_i_var[16]);
	_i_var[22] = (_i_var[19])*(_i_var[18]);
	_i_var[23] = (_i_var[15])*(_i_var[20]);
	_i_var[24] = (_i_var[16])*(_i_var[16]);
	_i_var[25] = (_i_var[22])*(_i_var[21]);
	_i_var[26] = (_i_var[24])*(_i_var[23]);
	_i_var[27] = (_i_var[6])*(_i_var[1]);
	_i_var[28] = (_i_var[6])*(_i_var[0]);
	_i_var[29] = (_i_var[26])+(_i_var[25]);
	_i_var[30] = (_i_var[22])*(_i_var[16]);
	_i_var[31] = (_i_var[27])*(_i_var[27]);
	_i_var[32] = (_i_var[28])*(_i_var[28]);
	_i_var[33] = (_i_var[27])*(_i_var[29]);
	_i_var[34] = (_i_var[30])*(_i_var[6]);
	_i_var[35] = (_i_var[31])*(_i_var[29]);
	_i_var[36] = (_i_var[32])*(_i_var[29]);
	_i_var[37] = (_i_var[28])*(_i_var[33]);
	_i_var[38] = (_i_var[35])+(_i_var[34]);
	_i_var[39] = (_i_var[36])+(_i_var[34]);
	_i_var[40] = (_i_var[18])*(_i_var[37]);
	_i_var[41] = (_i_var[18])*(_i_var[38]);
	_i_var[42] = (_i_var[18])*(_i_var[39]);
	_i_var[43] = (_i_var[18])*(_i_var[40]);
	energyhessian(0,0) = _i_var[38];
	energyhessian(1,0) = _i_var[37];
	energyhessian(2,0) = _i_var[41];
	energyhessian(3,0) = _i_var[40];
	energyhessian(0,1) = _i_var[37];
	energyhessian(1,1) = _i_var[39];
	energyhessian(2,1) = _i_var[40];
	energyhessian(3,1) = _i_var[42];
	energyhessian(0,2) = _i_var[41];
	energyhessian(1,2) = _i_var[40];
	energyhessian(2,2) = _i_var[38];
	energyhessian(3,2) = _i_var[43];
	energyhessian(0,3) = _i_var[40];
	energyhessian(1,3) = _i_var[42];
	energyhessian(2,3) = _i_var[43];
	energyhessian(3,3) = _i_var[39];
}

void computeRepulsion3DCubicEnergy(const Eigen::Matrix<double,3,1> & xi, const Eigen::Matrix<double,3,1> & xj, double dhat, double& energy){
	double _i_var[13];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (xi(2,0))-(xj(2,0));
	_i_var[3] = (_i_var[0])*(_i_var[0]);
	_i_var[4] = (_i_var[1])*(_i_var[1]);
	_i_var[5] = (_i_var[2])*(_i_var[2]);
	_i_var[6] = (_i_var[4])+(_i_var[3]);
	_i_var[7] = (_i_var[6])+(_i_var[5]);
	_i_var[8] = std::sqrt(_i_var[7]);
	_i_var[9] = (_i_var[8])-(dhat);
	_i_var[10] = 3;
	_i_var[11] = -(_i_var[9]);
	_i_var[12] = std::pow(_i_var[11],_i_var[10]);
	energy = _i_var[12];
}
void computeRepulsion3DCubicEnergyGradient(const Eigen::Matrix<double,3,1> & xi, const Eigen::Matrix<double,3,1> & xj, double dhat, Eigen::Matrix<double, 6, 1>& energygradient){
	double _i_var[30];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (xi(2,0))-(xj(2,0));
	_i_var[3] = (_i_var[0])*(_i_var[0]);
	_i_var[4] = (_i_var[1])*(_i_var[1]);
	_i_var[5] = (_i_var[2])*(_i_var[2]);
	_i_var[6] = (_i_var[4])+(_i_var[3]);
	_i_var[7] = (_i_var[6])+(_i_var[5]);
	_i_var[8] = std::sqrt(_i_var[7]);
	_i_var[9] = (_i_var[8])-(dhat);
	_i_var[10] = -(_i_var[9]);
	_i_var[11] = 2;
	_i_var[12] = (_i_var[10])*(_i_var[10]);
	_i_var[13] = 3;
	_i_var[14] = (_i_var[11])*(_i_var[8]);
	_i_var[15] = 1;
	_i_var[16] = -1;
	_i_var[17] = (_i_var[13])*(_i_var[12]);
	_i_var[18] = (_i_var[15])/(_i_var[14]);
	_i_var[19] = (_i_var[17])*(_i_var[16]);
	_i_var[20] = (_i_var[19])*(_i_var[18]);
	_i_var[21] = (_i_var[20])*(_i_var[1]);
	_i_var[22] = (_i_var[20])*(_i_var[0]);
	_i_var[23] = (_i_var[20])*(_i_var[2]);
	_i_var[24] = (_i_var[11])*(_i_var[21]);
	_i_var[25] = (_i_var[11])*(_i_var[22]);
	_i_var[26] = (_i_var[11])*(_i_var[23]);
	_i_var[27] = (_i_var[24])*(_i_var[16]);
	_i_var[28] = (_i_var[25])*(_i_var[16]);
	_i_var[29] = (_i_var[26])*(_i_var[16]);
	energygradient(0,0) = _i_var[24];
	energygradient(1,0) = _i_var[25];
	energygradient(2,0) = _i_var[26];
	energygradient(3,0) = _i_var[27];
	energygradient(4,0) = _i_var[28];
	energygradient(5,0) = _i_var[29];
}
void computeRepulsion3DCubicEnergyHessian(const Eigen::Matrix<double,3,1> & xi, const Eigen::Matrix<double,3,1> & xj, double dhat, Eigen::Matrix<double, 6, 6>& energyhessian){
	double _i_var[59];
	_i_var[0] = (xi(1,0))-(xj(1,0));
	_i_var[1] = (xi(0,0))-(xj(0,0));
	_i_var[2] = (xi(2,0))-(xj(2,0));
	_i_var[3] = (_i_var[0])*(_i_var[0]);
	_i_var[4] = (_i_var[1])*(_i_var[1]);
	_i_var[5] = (_i_var[2])*(_i_var[2]);
	_i_var[6] = (_i_var[4])+(_i_var[3]);
	_i_var[7] = (_i_var[6])+(_i_var[5]);
	_i_var[8] = std::sqrt(_i_var[7]);
	_i_var[9] = 2;
	_i_var[10] = (_i_var[9])*(_i_var[8]);
	_i_var[11] = (_i_var[10])*(_i_var[10]);
	_i_var[12] = 1;
	_i_var[13] = (_i_var[8])-(dhat);
	_i_var[14] = (_i_var[12])/(_i_var[11]);
	_i_var[15] = -(_i_var[13]);
	_i_var[16] = -(_i_var[14]);
	_i_var[17] = (_i_var[15])*(_i_var[15]);
	_i_var[18] = 3;
	_i_var[19] = (_i_var[12])/(_i_var[10]);
	_i_var[20] = (_i_var[16])*(_i_var[9]);
	_i_var[21] = -1;
	_i_var[22] = (_i_var[18])*(_i_var[17]);
	_i_var[23] = (_i_var[9])*(_i_var[15]);
	_i_var[24] = (_i_var[20])*(_i_var[19]);
	_i_var[25] = (_i_var[22])*(_i_var[21]);
	_i_var[26] = (_i_var[18])*(_i_var[23]);
	_i_var[27] = (_i_var[19])*(_i_var[19]);
	_i_var[28] = (_i_var[25])*(_i_var[24]);
	_i_var[29] = (_i_var[27])*(_i_var[26]);
	_i_var[30] = (_i_var[9])*(_i_var[1]);
	_i_var[31] = (_i_var[9])*(_i_var[0]);
	_i_var[32] = (_i_var[9])*(_i_var[2]);
	_i_var[33] = (_i_var[29])+(_i_var[28]);
	_i_var[34] = (_i_var[25])*(_i_var[19]);
	_i_var[35] = (_i_var[30])*(_i_var[30]);
	_i_var[36] = (_i_var[31])*(_i_var[31]);
	_i_var[37] = (_i_var[32])*(_i_var[32]);
	_i_var[38] = (_i_var[30])*(_i_var[33]);
	_i_var[39] = (_i_var[32])*(_i_var[33]);
	_i_var[40] = (_i_var[34])*(_i_var[9]);
	_i_var[41] = (_i_var[35])*(_i_var[33]);
	_i_var[42] = (_i_var[36])*(_i_var[33]);
	_i_var[43] = (_i_var[37])*(_i_var[33]);
	_i_var[44] = (_i_var[31])*(_i_var[38]);
	_i_var[45] = (_i_var[30])*(_i_var[39]);
	_i_var[46] = (_i_var[31])*(_i_var[39]);
	_i_var[47] = (_i_var[41])+(_i_var[40]);
	_i_var[48] = (_i_var[42])+(_i_var[40]);
	_i_var[49] = (_i_var[43])+(_i_var[40]);
	_i_var[50] = (_i_var[21])*(_i_var[44]);
	_i_var[51] = (_i_var[21])*(_i_var[45]);
	_i_var[52] = (_i_var[21])*(_i_var[46]);
	_i_var[53] = (_i_var[21])*(_i_var[47]);
	_i_var[54] = (_i_var[21])*(_i_var[48]);
	_i_var[55] = (_i_var[21])*(_i_var[49]);
	_i_var[56] = (_i_var[21])*(_i_var[50]);
	_i_var[57] = (_i_var[21])*(_i_var[51]);
	_i_var[58] = (_i_var[21])*(_i_var[52]);
	energyhessian(0,0) = _i_var[47];
	energyhessian(1,0) = _i_var[44];
	energyhessian(2,0) = _i_var[45];
	energyhessian(3,0) = _i_var[53];
	energyhessian(4,0) = _i_var[50];
	energyhessian(5,0) = _i_var[51];
	energyhessian(0,1) = _i_var[44];
	energyhessian(1,1) = _i_var[48];
	energyhessian(2,1) = _i_var[46];
	energyhessian(3,1) = _i_var[50];
	energyhessian(4,1) = _i_var[54];
	energyhessian(5,1) = _i_var[52];
	energyhessian(0,2) = _i_var[45];
	energyhessian(1,2) = _i_var[46];
	energyhessian(2,2) = _i_var[49];
	energyhessian(3,2) = _i_var[51];
	energyhessian(4,2) = _i_var[52];
	energyhessian(5,2) = _i_var[55];
	energyhessian(0,3) = _i_var[53];
	energyhessian(1,3) = _i_var[50];
	energyhessian(2,3) = _i_var[51];
	energyhessian(3,3) = _i_var[47];
	energyhessian(4,3) = _i_var[56];
	energyhessian(5,3) = _i_var[57];
	energyhessian(0,4) = _i_var[50];
	energyhessian(1,4) = _i_var[54];
	energyhessian(2,4) = _i_var[52];
	energyhessian(3,4) = _i_var[56];
	energyhessian(4,4) = _i_var[48];
	energyhessian(5,4) = _i_var[58];
	energyhessian(0,5) = _i_var[51];
	energyhessian(1,5) = _i_var[52];
	energyhessian(2,5) = _i_var[55];
	energyhessian(3,5) = _i_var[57];
	energyhessian(4,5) = _i_var[58];
	energyhessian(5,5) = _i_var[49];
}